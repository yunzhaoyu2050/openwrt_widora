--[[lit-meta
  ...
  from = "https://github.com/creationix/weblit/blob/master/libs/weblit-router.lua"
]]
...
local function newRouter()
    ...
    function router.use(path, handler)
    ...
    function router.route(options, handler)
        ...
        router.use(function(req, res, go)
            if method and req.method ~= method then
                return go()
            end
            if host and not host(req.headers.host) then
                return go()
            end
            if filter and not filter(req) then
                return go()
            end
            local params
            if path then
    ...
    function router.run(req, res, go)
        ...
        local function run(i)
            if i > len then
                local _, pos, key = req.path:find("^/(%w+)/")
                -- p(pos, key)
                if key ~= nil then
                    local handler = handlers[key]
                    if type(handler) == 'function' then
                        req.path = req.path:sub(pos)
                        return handler(req, res, go)
                    end
                end
                return (go or function()
                end)()
            end
