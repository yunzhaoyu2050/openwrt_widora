...
local function rand4()
  ...
  return char(rshift(num, 24), band(rshift(num, 16), 0xff), band(rshift(num, 8), 0xff), band(num, 0xff))
end
...
local function encode(item)
  ...
  local chars = {
    char(bor(fin and 0x80 or 0, rsv1 and 0x40 or 0, rsv2 and 0x20 or 0, rsv3 and 0x10 or 0, opcode)),
    char(bor(mask and 0x80 or 0, len < 126 and len or (len < 0x10000) and 126 or 127))
  }
  if len >= 0x10000 then
...
local function handleHandshake(head, protocol)
  ...
  if
    not (headers.connection and headers.upgrade and headers.connection:lower():find("upgrade", 1, true) and
      headers.upgrade:lower():find("websocket", 1, true))
   then
    return
  end
