...
function logging.prepareLogMsg(pattern, dt, level, message)
	local logMsg = nil
	if level == 'WARN' then
		-- logMsg = pattern or "[%date][\033[33m %level \033[0m]: %message\n"
		logMsg = pattern or "[%date][%level]: %message\n"
	elseif level == 'INFO' then
		-- logMsg = pattern or "[%date][\033[32m %level \033[0m]: %message\n"
		logMsg = pattern or "[%date][%level]: %message\n"
	elseif level == 'ERROR' then
		-- logMsg = pattern or "[%date][\033[31m% level \033[0m]: %message\n"
		logMsg = pattern or "[%date][%level]: %message\n"
		level = 'ERRO'
	elseif level == 'DEBUG' then
		-- logMsg = pattern or "[%date][\033[36m %level \033[0m]: %message\n"
		logMsg = pattern or "[%date][%level]: %message\n"
		level = 'DEBU'
	elseif level == 'FATAL' then
		-- logMsg = pattern or "[%date][\033[37m %level \033[0m]: %message\n"
		logMsg = pattern or "[%date][%level]: %message\n"
		level = 'FATA'
	else
		-- logMsg = pattern or "[%date][\033[37m %level \033[0m]: %message\n"
